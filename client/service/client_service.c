...
#include "network.h"
#include <openssl/crypto.h> // Necessario per OPENSSL_cleanse
#include <string.h>
#include <stdlib.h>
#include <unistd.h>
...
int client_service_has_ca() {
    return (access(CLIENT_CERTS_DIR "ca.crt", F_OK) == 0);
}

// =========================================================
// --- GESTIONE CRITTOGRAFIA (INTENT) E ON-THE-FLY ---
// =========================================================

int client_service_set_crypto_password() {
    // Passiamo NULL come credenziale, la password non viene immagazzinata!
    client_context_set_crypto_method(CRYPTO_METHOD_PASSWORD, NULL);
    return 0;
}

int client_service_set_crypto_usb_path(const char *path) {
    // Verifica che il file esista e sia leggibile prima di impostarlo
    if (!path || access(path, R_OK) != 0) return -1;
    client_context_set_crypto_method(CRYPTO_METHOD_USB, path);
    return 0;
}

int client_service_generate_new_usb_key(const char *path) {
    if (crypto_generate_usb_key(path) == 0) {
        // Se la generazione va a buon fine, impostiamo direttamente il percorso
        return client_service_set_crypto_usb_path(path);
    }
    return -1;
}

// Helper interno per derivare o caricare la chiave solo quando serve
static int derive_or_load_key_live(unsigned char *out_key, const char *live_pw) {
    CryptoMethod method = client_context_get_crypto_method();
    
    if (method == CRYPTO_METHOD_PASSWORD) {
        if (!live_pw || strlen(live_pw) == 0) return -1;
        
        // OpenSSL restituisce 1 in caso di successo! Lo convertiamo a 0 per il nostro standard.
        if (crypto_derive_from_password(live_pw, out_key) == 1) {
            return 0; // Successo
        }
        return -1; // Fallimento
        
    } else if (method == CRYPTO_METHOD_USB) {
        // Se è USB, recupera il percorso dal context
        const char *cred_path = client_context_get_crypto_credential();
        return crypto_load_usb_key(cred_path, out_key); // Questo restituisce già 0 al successo
    }
    return -1;
}
// =========================================================
...
// =========================================================
// --- STORE E FETCH VAULT (ON-THE-FLY) ---
// =========================================================
...
int client_service_store_data(const char *svc, const char *pass, const char *live_pw, char *out_server_resp, size_t resp_len) {
    ...
    // 1. Recupero chiave ON-THE-FLY passando live_pw
    unsigned char live_key[32]; // AES_KEY_LEN (256 bit)
    if (derive_or_load_key_live(live_key, live_pw) != 0) return -1;

    // 2. Cifratura
    unsigned char encrypted_blob[1024];
    int encrypted_len = crypto_encrypt((unsigned char*)pass, strlen(pass), live_key, encrypted_blob);

    // 3. ZEROIZATION DELLA CHIAVE IN RAM
    OPENSSL_cleanse(live_key, sizeof(live_key));

    if (encrypted_len < 0) return -1;

    // 4. Formattazione payload ed invio al server
...
int client_service_fetch_and_parse_vault(const char *live_pw, void (*data_handler)(const char *svc, const char *pass)) {
    SSL *ssl = client_context_get_ssl();
    if (!ssl || !client_context_is_crypto_ready() || !data_handler) return -1;

    // 1. Recupero chiave ON-THE-FLY passando live_pw
    unsigned char live_key[32];
    if (derive_or_load_key_live(live_key, live_pw) != 0) return -1;
    ...
    if (bytes <= 0) {
        OPENSSL_cleanse(live_key, sizeof(live_key));
        return 0; // Il vault è vuoto o il server non ha risposto con dati
    }

    int decryption_failed = 0;
    int parsed_items = 0;

    // Parsing e decifratura
    ...
    while (line != NULL) {
        ...
        if (svc && hex_payload && strlen(hex_payload) > 0) {
            ...
            // 2. Decifratura
            int decrypted_len = crypto_decrypt(ciphertext, blob_len, live_key, decrypted_pass);

            if (decrypted_len > 0) {
                decrypted_pass[decrypted_len] = '\0';
                data_handler(svc, (const char*)decrypted_pass);
                OPENSSL_cleanse(decrypted_pass, sizeof(decrypted_pass));
                parsed_items++;
            } else {
                decryption_failed = 1; // La password è sbagliata!
            }
            free(ciphertext);
        }
        line = strtok_r(NULL, "\n", &saveptr1);
    ...
    // 3. ZEROIZATION DELLA CHIAVE
    OPENSSL_cleanse(live_key, sizeof(live_key));
    
   // Se ha fallito la decifratura e NON ha recuperato nulla, allora la password è davvero sbagliata
    if (parsed_items == 0 && decryption_failed > 0) {
        return -1; 
    }
...
int client_service_is_using_password() {
    // Restituisce 1 (vero) se il metodo scelto è la password
    return (client_context_get_crypto_method() == CRYPTO_METHOD_PASSWORD);
}